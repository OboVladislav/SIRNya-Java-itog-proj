# Автоматический тест канала SMS.
# Поднимает фейковый SMPP-эмулятор (SMSC) с захватом вывода, регистрирует пользователя,
# генерирует OTP-код по каналу SMS, проверяет что SMS реально дошла до эмулятора,
# и подтверждает операцию этим кодом. Печатает PASS/FAIL.
# Перед запуском сервис должен работать: java -jar target/otp-service.jar
# Нужен собранный target/otp-service.jar (из него берётся библиотека jSMPP).
# sms.properties уже настроен на localhost:2775.
param(
